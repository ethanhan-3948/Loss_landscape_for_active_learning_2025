...
# noinspection DuplicatedCode
def batch_planar_interpolation(model_start: typing.Union[torch.nn.Module, ModelWrapper],
                         model_end_one: typing.Union[torch.nn.Module, ModelWrapper],
                         model_end_two: typing.Union[torch.nn.Module, ModelWrapper],
                         metric_list: typing.List[Metric], steps=20, deepcopy_model=False, scale=1, half = False) -> np.ndarray:
    """
    Similar to planar_interpolation but computes losses for multiple metrics at each perturbed model state.
    ...
    :param metric_list: list of Metric objects to evaluate model
    :param steps: at how many steps from start to end the model is evaluated
    :param deepcopy_model: indicates whether the method will deepcopy the model(s) to avoid aliasing
    :param scale: scale the direction vectors by a factor of scale
    :param half: if True, skip every other loss computation, this is to speed up the computation
    :return: 3D array of loss values with shape (steps, steps, num_samples)
    """
    model_start_wrapper = wrap_model(copy.deepcopy(model_start) if deepcopy_model else model_start)
    ...
    start_point = model_start_wrapper.get_module_parameters()
    dir_one = (model_end_one_wrapper.get_module_parameters()) / steps * scale
    dir_two = (model_end_two_wrapper.get_module_parameters()) / steps * scale
    ...
    data_matrix = []
    # evaluate loss in grid of (steps * steps) points
            ...
            for _ in range(steps):
                ...
                if i % 2 == 0:
                    ...
                    for metric in metric_list:
                        losses.append(metric(model_start_wrapper))

                    data_column.append(losses)
    ...
    # Move all tensors to cpu and convert to numpy before np.array
    data_matrix = [[[v.cpu().numpy() if hasattr(v, 'cpu') and hasattr(v, 'numpy') else v for v in sample] for sample in row] for row in data_matrix]
...
def planar_interpolation(model_start: typing.Union[torch.nn.Module, ModelWrapper],
                         model_end_one: typing.Union[torch.nn.Module, ModelWrapper],
                         model_end_two: typing.Union[torch.nn.Module, ModelWrapper],
                         metric: Metric, distance=1, steps=20, deepcopy_model=False) -> np.ndarray:
    """
    ...
    # Move all tensors to cpu and convert to numpy before np.array
    data_matrix = [[v.cpu().numpy() if hasattr(v, 'cpu') and hasattr(v, 'numpy') else v for v in row] for row in data_matrix]
    return np.array(data_matrix)
